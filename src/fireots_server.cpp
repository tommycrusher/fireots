...
int FireotsServer::run() {
	...
	while (true) {
		...
		if (now - lastLog >= warnEvery) {
			logger.warn("Startup still running ({} s)�Ǫ", std::chrono::duration_cast<std::chrono::seconds>(now - start).count());
			lastLog = now;
		}
...
void FireotsServer::validateDatapack() {
	...
	if (!useAnyDatapack && datapackName != "data-fire" && datapackName != "data-canary" && datapackName != "data-otservbr-global") {
		throw FailedToInitializeFireots(fmt::format(
			"The datapack folder name '{}' is wrong. Valid names: 'data-fire', 'data-canary', "

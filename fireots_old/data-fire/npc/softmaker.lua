local internalNpcName = "Laduje Softy"
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, 'soft') or MsgContains(message, ' soft boots') then
		npcHandler:say('Do you want to repair your worn soft boots for 10000 gold coins?',  npc, creature)
		npcHandler:setTopic(playerId, 1)
	elseif npcHandler:getTopic(playerId) == 1 then
		if(getPlayerItemCount(playerId, 6530) >= 1) then
			if(doPlayerRemoveMoney(playerId, 10000) == TRUE) then
				doPlayerRemoveItem(playerId, 6530, 1)
				doPlayerAddItem(playerId, 6529)

...
local underInv = Action()
function underInv.onUse(cid, item, frompos, item2, topos)
	if getPlayerAccess(cid) ~=0 then
		topleft={x=227, y=2073, z=7}
		bottonright={x=253, y=2116, z=7}
		maxmonsters= 45
		...
		repeat
			...
			if spawnrate==0 and summonedtotal ~= maxmonsters then
				...
				elseif species == 2 then
					checkforsummon = doSummonCreature("Hellhound",checking)
				elseif species == 3 then
					checkforsummon = doSummonCreature("Undead Dragon",checking)
